...
import { Filter, Calendar, FileDown, LogIn } from "lucide-react";
import * as XLSX from "xlsx";
import DatePicker from "react-multi-date-picker";
import persian from "react-date-object/calendars/persian";
import persian_fa from "react-date-object/locales/persian_fa";
import { useTheme } from "../../context/ThemeContext";
import PaymentsVisualization from "./PaymentChart";
  ...
  return (
    ...
    >
      ...
      <td className="p-3 text-right">
        <span
          className={`px-3 py-1 text-xs font-bold text-white bg-green-500 rounded-full shadow ${
            payment?.payment_status !== "Completed" ? "bg-red-500" : ""
          }`}
        >
...
const GymPaymentLogsPage = () => {
  ...
  return (
    ...
    >
      ...
      >
        <h1
          className={`text-2xl text-${background} font-bold text-center flex-1`}
        >
          لیست پرداخت‌ها
        </h1>
        <div className="flex flex-row gap-4">
          ...
          >
            ...
            <span className="text-sm">ورود به چارت</span>
          </button>
        </div>
      </header>

      {isChartOpen ? (
        <PaymentsVisualization />
      ) : (
        <>
          {/* Filters Section */}
          <div
